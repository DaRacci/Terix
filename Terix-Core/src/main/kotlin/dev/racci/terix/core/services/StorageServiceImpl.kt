...
import dev.racci.terix.api.data.player.TerixPlayerEntityHolder
...
public class StorageServiceImpl(override val plugin: Terix) : StorageService, Extension<Terix>() {
    override val managedTable: Table by lazy { TerixPlayerEntityHolder.table } // Lazy to prevent exception on init

    override suspend fun handleEnable() {
        event<AsyncPlayerPreLoginEvent>(EventPriority.MONITOR, true) {
            withDatabase { TerixPlayerEntityHolder.findById(uniqueId) ?: TerixPlayerEntityHolder.new(uniqueId) {} }
        }
    }
}
