...
public class OptionL {
    ...
    private static final Map<Option, OptionValue> options = new EnumMap<>(Option.class);

    public OptionL(Sylphia plugin) {
        this.plugin = plugin;
    }
        ...
        for (Option option : Option.values()) {
            ...
            if (value != null) {
                ...
                //Error
                    ...
                    Logger.log(Logger.Level.WARNING, "Incorrect type in config.yml: Option " + option.name() + " with path " + option.getPath() + " should be of type " + option.getType().name() + ", using default value instead!");
                }
            }
            else {
                Logger.log(Logger.Level.WARNING,"Missing value in config.yml: Option " + option.name() + " with path " + option.getPath() + " was not found, using default value instead!");
            }
        }
        long end = System.currentTimeMillis();
        Logger.log(Logger.Level.WARNING,"Loaded " + loaded + " config options in " + (end - start) + " ms");
    }
